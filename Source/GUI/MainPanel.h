...
//  MainPanel — 1画面統合パネル
//
//  上段 : DynVisual (Stage1 / Stage2 横並び)
//  中段 : PRE-DRIVE / CROSSOVER / MASTER を 1 行に集約
//  下段 : Stage1 と Stage2 を左右に同時表示。
//         各ステージは「3行 (LOW/MID/HIGH) × 5列 (GAIN/UP/DN/ATK/REL)」の行列。
//         行頭と列頭にラベルを 1 回ずつ置くことで、ノブ個別のラベル 15 個が不要になり、
//         同じ面積でノブ直径を約 2 倍にできる。
// ============================================================================
...
public:
    MainPanel (juce::AudioProcessorValueTreeState& apvts, MultiOtoLookAndFeel& laf)
    {
        ...
        // 表示テキストのみ変更。パラメータ側の選択肢文字列 ("2".."128") は不変なので
        // ホストのオートメーション互換は保たれる (Attachment は index で同期する)。
        ...
        const auto cXover  = MOColors::babyBlue;
        ...
        xLow.build      (apvts, "xover_low",   "LOW X",    this, laf, cXover);
        xHigh.build     (apvts, "xover_high",  "HIGH X",   this, laf, cXover);
        ...
        // --- Stage 行列 ---
        buildStage (apvts, laf, 1, s1Gain, s1Up, s1Dn, s1Time, s1Mix, s1Atk, s1Rel, MOColors::peach);
        buildStage (apvts, laf, 2, s2Gain, s2Up, s2Dn, s2Time, s2Mix, s2Atk, s2Rel, MOColors::babyBlue);
    ...
    void paint (juce::Graphics& g) override
    {
        ...
        g.drawText ("PRE-DRIVE / CROSSOVER / MASTER", globalLabelArea, juce::Justification::centredLeft);
    ...
    void resized() override
    {
        ...
        ArcKnob* globals[] = { &inGain, &drive, &oddBlend, &evenBlend, &xLow, &xHigh,
...
private:
    ...
    void buildStage (juce::AudioProcessorValueTreeState& apvts, MultiOtoLookAndFeel& laf, int s,
                     ...
                     ArcKnob* ak, ArcKnob* rl, juce::Colour macroColour)
    {
    ...
    void layoutStage (int idx, juce::Rectangle<int> b)
    {
        ...
        (idx == 0 ? s1OnBtn : s2OnBtn).setBounds (titleRow.removeFromRight (56).withSizeKeepingCentre (52, 18));

        inner.removeFromTop (4);
        auto macroRow = inner.removeFromTop (18);
        const int mw = (macroRow.getWidth() - 8) / 2;
        (idx == 0 ? s1Time : s2Time).slider.setBounds (macroRow.removeFromLeft (mw));
        macroRow.removeFromLeft (8);
        (idx == 0 ? s1Mix : s2Mix).slider.setBounds (macroRow.removeFromLeft (mw));
    ...
    ArcKnob inGain, drive, oddBlend, evenBlend, xLow, xHigh;
