...
Crossover::Crossover() {
    auto setType = [](juce::dsp::LinkwitzRileyFilter<float>& f, juce::dsp::LinkwitzRileyFilterType type) { f.setType(type); };
    setType(lp1L, juce::dsp::LinkwitzRileyFilterType::lowpass); setType(hp1L, juce::dsp::LinkwitzRileyFilterType::highpass);
    setType(lp1R, juce::dsp::LinkwitzRileyFilterType::lowpass); setType(hp1R, juce::dsp::LinkwitzRileyFilterType::highpass);
    setType(lp2L, juce::dsp::LinkwitzRileyFilterType::lowpass); setType(hp2L, juce::dsp::LinkwitzRileyFilterType::highpass);
    setType(lp2R, juce::dsp::LinkwitzRileyFilterType::lowpass); setType(hp2R, juce::dsp::LinkwitzRileyFilterType::highpass);
}
...
void Crossover::process(float inL, float inR, float& lowL, float& lowR, float& midL, float& midR, float& highL, float& highR) {
    lowL = lp1L.processSample(0, inL); lowR = lp1R.processSample(1, inR);
    float remL = hp1L.processSample(0, inL); float remR = hp1R.processSample(1, inR);
...
void Crossover::processDry(float inL, float inR, float& dryL, float& dryR) {
    dryL = inL; dryR = inR; // Align Phaseは無効化
}