...
public:
    ...
    {
        // ------------------------------------------------------------------
        // 帯域ゲインの変調深さ = ノブのレンジそのもの (±24dB)。段数では割らない。
        //
        // 【なぜ段数で割らないのか】
        // 以前は「ゲインは各ノードで適用されるから効果は段数に比例するはず」と
        // 考えて ±48dB / 段数 にしていた。理屈上はカスケード合計が一定になる。
        // だが実際に鳴らすと x2 以外はまるで変化が感じられなかった。
        // 理由は各ノードの間にコンプレッサーが挟まっているため。ノードが持ち上げた
        // 分をダウンワード側が即座に引き戻すので、ゲインは段数分そのまま積み上がら
        // ない。つまり「合計 dB」は体感と一致しない指標だった。
        //
        // 体感に効くのは 1 段あたりの深さそのもの。よってどの段数でも同じ
        // ±24dB を掛ける。x2 で良かった効き方が x128 でもそのまま得られる。
        //
        // 【安全性】呼び出し側 (PluginProcessor::readModParams) がパラメータの
        // レンジ [-24,+24] にクランプするので 1 段あたりの上限は元の GAIN ノブを
        // 振り切ったときと同じ。さらに DynamicsNode が入力を ±kSafeCeiling
        // (1e6 = +120dBFS) でクランプするため、段数を重ねても Inf/NaN には到達
        // しない。最終段のリミッターが CEILING を保証する。
        // 段数が多いほど暴れるのは仕様どおり (16 段以上は破壊領域)。
        // 大人しくしたいときは AMT を下げること。
        // ------------------------------------------------------------------
        juce::ignoreUnused (nodeCount);
        if (dst >= DstS1GainL && dst <= DstS2GainH)
            return baseValue + mod * 24.0;
