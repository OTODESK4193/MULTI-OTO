...
void EngineCore::prepare(double sampleRate, int samplesPerBlock) {
    ...
    driveSmoother.reset(sampleRate, 0.05); oddSmoother.reset(sampleRate, 0.05);
    evenSmoother.reset(sampleRate, 0.05); inGainSmoother.reset(sampleRate, 0.05);
    outGainSmoother.reset(sampleRate, 0.05);
    reset(); isPrepared.store(true, std::memory_order_release);
}
...
void EngineCore::updateParameters(const EngineParams& p) {
    currentParams = p;
    driveSmoother.setTargetValue(p.drive); oddSmoother.setTargetValue(p.odd);
    evenSmoother.setTargetValue(p.even); inGainSmoother.setTargetValue(FastMath::fast_exp2(p.inGain * 0.16609f));
    ...
    // 【追加】mix パラメータを各ノードに渡す
    int half = p.total_ott_count / 2;
    for (int i = 0; i < half; ++i) {
        nodes[i].setParameters(p.s1_gain, p.s1_depth, p.s1_time, p.s1_atk, p.s1_rel, p.s1_mix);
    }
    for (int i = half; i < p.total_ott_count; ++i) {
        nodes[i].setParameters(p.s2_gain, p.s2_depth, p.s2_time, p.s2_atk, p.s2_rel, p.s2_mix);
    }
...
void EngineCore::process(juce::AudioBuffer<float>& buffer) {
    ...
    dryBuffer.clear();
    float* dryLeft = dryBuffer.getWritePointer(0); float* dryRight = dryBuffer.getWritePointer(1);
    float mix = currentParams.dryWet * 0.01f;

    for (int i = 0; i < numSamples; ++i) {
        if (currentParams.phase_mode == 1) crossover.processDry(left[i], right[i], dryLeft[i], dryRight[i]);
        else { dryLeft[i] = left[i]; dryRight[i] = right[i]; }
    }

    float inGainTgt = inGainSmoother.getNextValue(); inGainSmoother.skip(numSamples - 1);
    ...
    // 【追加】PreDriveのバイパス処理
    float driveTgt = driveSmoother.getNextValue(); float oddTgt = oddSmoother.getNextValue(); float evenTgt = evenSmoother.getNextValue();
    driveSmoother.skip(numSamples - 1); oddSmoother.skip(numSamples - 1); evenSmoother.skip(numSamples - 1);

    if (currentParams.predrive_on && (driveTgt > 0.01f || oddTgt > 0.01f || evenTgt > 0.01f)) {
        for (int i = 0; i < numSamples; ++i) {
            left[i] = preLpfL.processSample(0, left[i]); right[i] = preLpfR.processSample(1, right[i]);
        }
    ...
    // 【追加】Stage 1 と Stage 2 のバイパス処理
    int half = currentParams.total_ott_count / 2;
    for (int n = 0; n < currentParams.total_ott_count; ++n) {
        if (n < half && currentParams.s1_on) nodes[n].process(simdBuffer.data(), numSamples);
        else if (n >= half && currentParams.s2_on) nodes[n].process(simdBuffer.data(), numSamples);
    }

    float outGainTgt = outGainSmoother.getNextValue(); outGainSmoother.skip(numSamples - 1);
    for (int i = 0; i < numSamples; ++i) {
        alignas(32) float raw[8]; simdBuffer[static_cast<size_t>(i)].copyToRawArray(raw);
        float wetL = raw[0] + raw[2] + raw[4]; float wetR = raw[1] + raw[3] + raw[5];
        ...
        left[i] = outL * gainL; right[i] = outR * gainR;
    }
}