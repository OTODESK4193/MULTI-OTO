...
public:
    ...
    void reset() {
        v_envSq = _mm256_setzero_ps(); // RMS用の2乗エンベロープ状態
    }
    ...
    void process(juce::dsp::SIMDRegister<float>* simmData, int numSamples) {
        ...
        const __m256 down_ratio_m = _mm256_set1_ps(-(1.0f - 0.25f)); // 1:4
        const __m256 up_ratio_m = _mm256_set1_ps(0.5f);              // 1:2

        // 真のOTTの要: Upward Range (最大+36dBまでの持ち上げに制限)
        const __m256 max_up_gain = _mm256_set1_ps(36.0f);
        ...
        for (int i = 0; i < numSamples; ++i) {
            ...
            // ルート計算でRMS値を求め、0割りを防ぐ
            __m256 safeEnvSq = _mm256_max_ps(v_envSq, _mm256_set1_ps(1e-15f));
            __m256 envRms = _mm256_sqrt_ps(safeEnvSq);
