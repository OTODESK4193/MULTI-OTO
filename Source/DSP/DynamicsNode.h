...
public:
    ...
    void setParameters(const float* gains, const float* depths, const float* upWards, const float* downWards, float macroTime, const float* attacks, const float* releases, float mix) {
        float timeScale = macroTime * 0.01f;
        alignas(32) float aArr[8] = { 1, 1, 1, 1, 1, 1, 1, 1 }, rArr[8] = { 1, 1, 1, 1, 1, 1, 1, 1 };
        alignas(32) float upArr[8] = { 0, 0, 0, 0, 0, 0, 0, 0 }, dnArr[8] = { 0, 0, 0, 0, 0, 0, 0, 0 }, gArr[8] = { 1, 1, 1, 1, 1, 1, 1, 1 };
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            float dVal = depths[i] * 0.01f;
            upArr[i * 2] = dVal * (upWards[i] * 0.01f); upArr[i * 2 + 1] = upArr[i * 2];
            dnArr[i * 2] = dVal * (downWards[i] * 0.01f); dnArr[i * 2 + 1] = dnArr[i * 2];

            float gVal = std::pow(10.0f, gains[i] / 20.0f);
            gArr[i * 2] = gVal; gArr[i * 2 + 1] = gVal;
        }
        v_atk = _mm256_load_ps(aArr); v_rel = _mm256_load_ps(rArr);
        v_up_depth = _mm256_load_ps(upArr); v_down_depth = _mm256_load_ps(dnArr);
    ...
    void process(juce::dsp::SIMDRegister<float>* simmData, int numSamples) {
        ...
        for (int i = 0; i < numSamples; ++i) {
            ...
            __m256 upGainDb = _mm256_mul_ps(cappedUpGain, v_up_depth);
    ...
    void getLastMeter (float outEnvDb[3], float outGainDb[3]) const
    {
        ...
        for (int b = 0; b < 3; ++b)
        {
            ...
            alignas(32) float upArr[8], dnArr[8], gArr[8];
            _mm256_store_ps (upArr, v_up_depth);
            _mm256_store_ps (dnArr, v_down_depth);
            _mm256_store_ps (gArr, v_gain);

            float downGain = downOver * (-(1.0f - 0.25f)) * dnArr[b * 2];
            float upGain   = std::min (upUnder * 0.5f, 36.0f) * upArr[b * 2];
...
private:
    ...
    __m256 v_envSq, v_atk, v_rel, v_up_depth, v_down_depth, v_gain, v_mix;
};