...
Frequencies and times modulate **exponentially** (±2 to ±3 octaves) rather than by addition. Adding ±500 Hz to an 88 Hz crossover would collapse the downward side; ±2 octaves swings symmetrically from 22 Hz to 352 Hz. Band gain is the special case: because it compounds with the node count, its modulation depth is divided by that count — `±48 ÷ count` dB per node, which lands at ±24 dB within a stage and ±48 dB across the full chain no matter what count you pick. Otherwise ±3 dB at 128 nodes would mean ±384 dB.

Every modulated control shows its live range directly on the ring with a marker at the current position, and modulated band gains also show their range on the meters. The display runs through the same function the DSP uses, so what you see is what you hear.
