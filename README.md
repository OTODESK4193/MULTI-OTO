...
👉 **[Demo video on X (デモ動画)](https://x.com/kijyoumusic/status/2086085714962485738?s=20)**
...
Frequencies and times modulate **exponentially** (±2 to ±3 octaves) rather than by addition. Adding ±500 Hz to an 88 Hz crossover would collapse the downward side; ±2 octaves swings symmetrically from 22 Hz to 352 Hz. Band gain is a flat ±24 dB — the GAIN knob's own range — at every cascade count. An earlier build divided that depth by the node count on the assumption that per-node gain accumulates across the chain; it does not, because a compressor sits between every pair of nodes and pulls back whatever the previous one lifted. Per-node depth is what you actually hear.

Every modulated control shows its live range directly on the ring with a marker at the current position, and modulated band gains show their range on the meters while modulated crossovers move the meter band boundaries themselves. The display runs through the same function the DSP uses, so what you see is what you hear.
